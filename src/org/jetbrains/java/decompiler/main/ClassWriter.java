...
public class ClassWriter {
      ...
      if ((flags & (CodeConstants.ACC_ABSTRACT | CodeConstants.ACC_NATIVE)) != 0) { // native or abstract method (explicit or interface)
        ...
        if (root != null && !methodWrapper.decompiledWithErrors) { // check for existence
          try {
            ...
            hideMethod = (clinit || dinit || hideConstructor(cl, init, throwsExceptions, paramCount, clFlags, flags)) && code.length() == 0;
  ...
  private static boolean hideConstructor(StructClass cl, boolean init, boolean throwsExceptions, int paramCount, int clFlags, int mtFlags) {
    if (!init || throwsExceptions || paramCount > 0 || !DecompilerContext.getOption(IFernflowerPreferences.HIDE_DEFAULT_CONSTRUCTOR)) {
      return false;
    }

    int clAccess = clFlags & ACCESSIBILITY_FLAGS;
    int mtAccess = mtFlags & ACCESSIBILITY_FLAGS;
    boolean decompileEnum = DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_ENUM);

    // default constructor requires same accessibility flags. Exception: enum constructor which is always private
    if (((clFlags & (CodeConstants.ACC_ENUM | CodeConstants.ACC_ABSTRACT)) == 0 || !decompileEnum) && clAccess != mtAccess)
  ...
  public static void appendTypeParameters(TextBuffer buffer, List<String> parameters, List<? extends List<VarType>> bounds) {
    buffer.append('<');
